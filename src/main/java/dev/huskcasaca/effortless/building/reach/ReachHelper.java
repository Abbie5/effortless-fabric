...
public class ReachHelper {
    ...
    public static ReachConfig getReachSettings(Player player) {
        return ((EffortlessDataProvider) player).getReachSettings();
    }

    public static void setReachSettings(Player player, ReachConfig reachConfig) {
        if (player == null) {
            Effortless.log("Cannot set reach settings, player is null");
            return;
        }
        ((EffortlessDataProvider) player).setReachSettings(reachConfig);
    }

    public static String getSanitizeMessage(ReachConfig modeSettings, Player player) {
    ...
    public static ReachConfig sanitize(ReachConfig reachConfig, Player player) {
//        maxBlockPlaceAtOnce = Math.toIntExact(Math.round(maxBlockPlaceAtOnce / 1000.0)) * 1000;
        return new ReachConfig(
                Math.max(MIN_MAX_REACH_DISTANCE, Math.min(reachConfig.maxReachDistance(), MAX_MAX_REACH_DISTANCE)),
                Math.max(MIN_MAX_BLOCK_PLACE_PER_AXIS, Math.min(reachConfig.maxBlockPlacePerAxis(), MAX_MAX_BLOCK_PLACE_PER_AXIS)),
                Math.max(MIN_MAX_BLOCK_PLACE_AT_ONCE, Math.min(reachConfig.maxBlockPlaceAtOnce(), MAX_MAX_BLOCK_PLACE_AT_ONCE)),
                reachConfig.canBreakFar(),
                reachConfig.enableUndoRedo(),
                Math.max(MIN_UNDO_STACK_SIZE, Math.min(reachConfig.undoStackSize(), MAX_UNDO_STACK_SIZE))
        );
    }

    public static void handleNewPlayer(ServerPlayer player) {
//        if (!player.level.isClientSide) {
//            Packets.sendToClient(new ClientboundPlayerSettingsPacket(((EffortlessDataProvider) player).getReachSettings()), player);
//        }
    }
...
//            Packets.sendToClient(new ClientboundPlayerSettingsPacket(getReachSettings(player)), (ServerPlayer) player);
//        } else {
//            Packets.sendToServer(new ServerboundPlayerSettingsPacket(getReachSettings(player)));
//        }
    }
